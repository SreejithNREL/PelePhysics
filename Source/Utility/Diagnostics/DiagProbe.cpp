...
amrex::Real
LinearInterpolate (	amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> xp,
...
DiagProbe::init(const std::string& a_prefix, std::string_view a_diagName)
{
  ...
  if (probe_loc.size() >= AMREX_SPACEDIM) {
    for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
	  ...
	  amrex::Abort("\nProvide probe location array with same dimension as problem dimension");
  }
  ...
  if (intType == "Linear") {
    ...
    amrex::Abort("Unknown interpolation type for " + a_prefix+". Allowed values are Linear or CellCenter.\n");
  }

  //Set output file properties
  amrex::UtilCreateDirectory("temporals", 0755);
  std::string tmpProbeFileName = "temporals/Probe_"+std::string(a_diagName)+".out";
  if (amrex::ParallelDescriptor::IOProcessor()) {
  tmpProbeFile.open(tmpProbeFileName.c_str(), std::ios::out | std::ios::app | std::ios_base::binary);
  ...
  const amrex::Vector<std::string>& a_varNames)
{
	//Check if probe lies within the geometry
	 if(!(a_geoms[0].insideRoundoffDomain(AMREX_D_DECL(m_probe_loc[0],m_probe_loc[1], m_probe_loc[2]))))
	 {
	 ...
	 for (int lev = a_nlevels-1; lev >=0; lev--)
	 {
	    ...
	    amrex::Print()<<"\n Dist = "<<dist[0]<<" after static cast "<<static_cast<int>(dist[0]);
	    ...
	    for (int i =0;i<a_grids[lev].size();i++)
	    {
	    	...
	    	if (cBox.contains(idx_lev) && !probe_found) {
	    		//box found. store the lev, box number and box reference. set probe_found to true to stop searching any further
	    		...
	    		amrex::Print()<<"\nProbe found in lev = "<<lev<<" box = "<<i<<" idx = "<<m_probe_idx[0]<<" "<<m_probe_idx[1]<<"\n";
	    		probe_found=true;
	    	}
	    }
	 }
    ...
    const amrex::Vector<std::string>& a_varNames)
{
	...
	for (amrex::MFIter mfi(planeData[0], amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
	{
	      ...
	      {
	    	  amrex::Print()<<"\n N = "<<n<<" "<<idx_d_p[n];
	    	  int stIdx = idx_d_p[n];	//Sreejith. Need to change this to make more generic
...
#else
	    	  {
	    		  cell_data(0,0,0) = state(m_probe_idx[0],    m_probe_idx[1],     m_probe_idx[2], stIdx);
	    		  cell_data(1,0,0) = state(m_probe_idx[0]+1,m_probe_idx[1],     m_probe_idx[2], stIdx);
	    		  cell_data(0,1,0) = state(m_probe_idx[0],    m_probe_idx[1]+1, m_probe_idx[2], stIdx);
	    		  cell_data(1,1,0) = state(m_probe_idx[0]+1,m_probe_idx[1]+1, m_probe_idx[2], stIdx);

	    		  cell_data(0,0,1) = state(m_probe_idx[0],    m_probe_idx[1],     m_probe_idx[2]+1, stIdx);
	    		  cell_data(1,0,1) = state(m_probe_idx[0]+1,m_probe_idx[1],     m_probe_idx[2]+1, stIdx);
	    		  cell_data(0,1,1) = state(m_probe_idx[0],    m_probe_idx[1]+1, m_probe_idx[2]+1, stIdx);
	    		  cell_data(1,1,1) = state(m_probe_idx[0]+1,m_probe_idx[1]+1, m_probe_idx[2]+1, stIdx);
	    	  }
#endif
	    	  amrex::Real interpolatedval = LinearInterpolate(m_probe_loc,x_low_cell,cell_data, dx_finest_lev_probe);
...
#else
	    		  m_values_at_probe[n] = state(m_probe_idx[0],m_probe_idx[1], m_probe_idx[2], stIdx);
#endif
	    	  }
